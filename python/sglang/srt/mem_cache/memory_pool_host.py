...
class MambaPoolHost(HostKVCache):
    ...
    def _init_write_back_staging_buffers(self):
        ...
        # Both directions use the mapped-host Mamba kernels directly. The
        # kernels accept the real device row stride, so unified rows do not
        # need a gather buffer even though consecutive rows are separated by
        # the complete shared slot envelope.
    ...
    def _copy_tensor_pf_lf(
        ...
        if io_backend == "kernel":
            ...
            if dst.device.type != "cuda":
                selected = src[:, layer_id, 0].index_select(
                    0, src_indices.to(device="cpu", dtype=torch.long)
                )
                dst.index_copy_(0, dst_indices.to(dst.device), selected.to(dst.device))
                return
            ...
            transfer_kv_mamba_pf_lf(
                ...
                layer_id=layer_id,
                item_size=item_size,
                src_layout_dim=src.stride(0) * src.element_size(),
    ...
    def _copy_tensor_all_layers_lf_pf(
        ...
        if io_backend == "kernel":
            ...
            transfer_kv_mamba_lf_pf(
                ...
                dst_indices=dst_indices,
                item_size=item_size,
                dst_layout_dim=dst.stride(0) * dst.dtype.itemsize,
