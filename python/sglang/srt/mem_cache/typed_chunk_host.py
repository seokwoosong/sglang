    ...
    * Mamba index = ``chunk_id``
    ...
    def clear(self) -> None:
        with self._lock:
            ...
            self._free_chunks = list(range(self.num_chunks))
            heapq.heapify(self._free_chunks)
    ...
    def _pop_free_chunk(self) -> Optional[int]:
        while self._free_chunks:
            chunk_id = heapq.heappop(self._free_chunks)
    ...
    def _release_chunk(self, chunk_id: int) -> None:
        ...
        if chunk_id not in self._free_chunk_set:
            ...
            heapq.heappush(self._free_chunks, chunk_id)
...
def build_shared_kv_envelope_view(
    ...
    *,
    ...
    """Return ``[K/V, token, layer, head, dim]`` over slot envelopes.

    The physical order of one token is ``[L0 K, L0 V, L1 K, L1 V, ...]``.
    This is the same page-major envelope used by unified GPU memory at
    ``page_size == 1``.  K/V layer views have a token stride equal to the full
    KV entry, which the stride-aware HiCache JIT kernels can consume.
    """

    row_elems = head_num * head_dim
    entry_elems = 2 * layer_num * row_elems
    needed_bytes = num_tokens * entry_elems * dtype.itemsize
    ...
    return torch.as_strided(
        typed,
        size=(2, num_tokens, layer_num, head_num, head_dim),
        stride=(
            row_elems,
            entry_elems,
            2 * row_elems,
...
def build_shared_mamba_envelope_views(
    ...
    *,
    ...
    One chunk stores ``temporal`` followed by every conv component.  Each
    component keeps all layers contiguous so a selected slot can be copied with
    one asynchronous DMA per component.
    """
