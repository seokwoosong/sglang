    ...
    for kv_pool in kv_pools:
        ...
        if size_bytes == 0:
            ...
            if unified_buffer is not None and sub_pool_name is not None:
                ...
                size_bytes = (
                    unified_buffer.max_slots(sub_pool_name) * spec.entry_bytes()
                )
    ...
    *,
    ...
    if is_unified_mamba:
        full_allocator = getattr(
            params.token_to_kv_pool_allocator, "full_attn_allocator", None
        )
        shared_mamba_allocator = getattr(
            params.token_to_kv_pool_allocator, "mamba_allocator", None
        )
    ...
    if _use_unified_typed_l2(is_unified_mamba):
        ...
        if mtp_draft_device_pools:
            raise ValueError(
                "unified typed-chunk HiCache does not support speculative draft pools"
            )
