...
class MultiEndedAllocator(BaseTokenToKVPoolAllocator):
    ...
    def _flush_impl(self, *, urgent: bool) -> int:
        ...
        Pipeline (one free-list D2H plus one mapping D2H per committed batch):
          ...
          7. survivor walk — TWO-POINTER: move topmost live slot into the next hole,
             ...
             gathers and validates all survivor virtual ids in one operation.
        ...
        `_topmost_survivor` excludes all p2v=-1 pages, so a negative virtual id
        in the batched mapping lookup is a corrupt-state bug and raises.
        """
        if not self.lazy_compaction:
            return 0
    ...
    def _commit_move_batch(
        ...
        batch. Fired srcs accumulate in `released_fired` (merged by `_flush` AFTER
        its dst-slice, keeping the free list == holes_cpu); event-pending srcs
        route to `_pending_reuse` (read-race gating).
        """
        if not srcs:
            return
