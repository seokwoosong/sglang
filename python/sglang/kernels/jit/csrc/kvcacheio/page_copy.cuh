...
__global__ void hicache_page_copy_kernel(const __grid_constant__ HiCachePageCopyParams params) {
  ...
  for (uint64_t linear = thread_id; linear < total_vectors; linear += thread_stride) {
    ...
    const auto* src = reinterpret_cast<const pack_t*>(
        pointer::offset(params.src, static_cast<int64_t>(src_page * params.page_stride_bytes + inner_offset)));
    auto* dst = reinterpret_cast<pack_t*>(
        pointer::offset(params.dst, static_cast<int64_t>(dst_page * params.page_stride_bytes + inner_offset)));
...
struct HiCachePageCopyKernel {
  ...
  run(const tvm::ffi::TensorView dst,
      ...
      const int64_t page_stride_bytes,
    ...
    RuntimeCheck(page_stride_bytes > 0, "HiCache page stride must be positive");
    ...
    RuntimeCheck(
        copy_offset_bytes + copy_bytes <= page_stride_bytes, "HiCache page copy range exceeds the page envelope");
    RuntimeCheck(
        page_stride_bytes % 16 == 0 && copy_offset_bytes % 16 == 0 && copy_bytes % 16 == 0,
    ...
    const auto params = HiCachePageCopyParams{
        ...
        .page_stride_bytes = static_cast<uint64_t>(page_stride_bytes),
