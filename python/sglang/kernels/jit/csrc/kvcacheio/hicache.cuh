...
inline void* device_accessible_ptr(const tvm::ffi::TensorView& tensor) {
  ...
  if (type != kDLCPU && type != kDLGPUHost) return ptr;

  void* device_ptr = nullptr;
#ifndef USE_ROCM
  const auto error = cudaHostGetDevicePointer(&device_ptr, ptr, 0);
  RuntimeCheck(error == cudaSuccess, "HiCache: failed to resolve registered host pointer: ", cudaGetErrorString(error));
#else
  const auto error = hipHostGetDevicePointer(&device_ptr, ptr, 0);
  RuntimeCheck(error == hipSuccess, "HiCache: failed to resolve registered host pointer: ", hipGetErrorString(error));
#endif
  return device_ptr;
}
...
struct HiCacheKernel {
  ...
  static void run_one(
    ...
    const auto k_cache_dst_ptr = host::device_accessible_ptr(k_cache_dst);
    const auto v_cache_dst_ptr = host::device_accessible_ptr(v_cache_dst);
    const auto k_cache_src_ptr = host::device_accessible_ptr(k_cache_src);
    const auto v_cache_src_ptr = host::device_accessible_ptr(v_cache_src);
  ...
  static void run_one_mla(
    ...
    const auto cache_dst_ptr = host::device_accessible_ptr(cache_dst);
    const auto cache_src_ptr = host::device_accessible_ptr(cache_src);
    const auto indices_dst_ptr = indices_dst.data_ptr();
    const auto indices_src_ptr = indices_src.data_ptr();
    const auto length = static_cast<uint32_t>(L.unwrap());
