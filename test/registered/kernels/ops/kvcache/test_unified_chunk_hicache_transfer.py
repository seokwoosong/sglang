...
def test_raw_page_copy_supports_different_source_and_destination_strides():
    src_stride = 256
    dst_stride = 384
    num_pages = 6
    arena = SharedTypedChunkHostArena(
        total_bytes=num_pages * dst_stride,
        kv_page_bytes=128,
        mamba_slot_bytes=dst_stride,
        host_device="cpu",
        accelerator_device="cuda",
        pin_memory=True,
        allocator_type="default",
    ...
    try:
        source = (
            torch.arange(num_pages * src_stride, dtype=torch.int64, device="cuda")
            .remainder_(251)
            .to(torch.uint8)
        )
        arena.raw.fill_(255)
        source_pages = torch.tensor([1, 3, 5], dtype=torch.int64, device="cuda")
        destination_pages = torch.tensor([0, 2, 4], dtype=torch.int64, device="cuda")
        transfer_hicache_pages(
            arena.raw,
            destination_pages,
            source,
            source_pages,
            dst_page_stride_bytes=dst_stride,
            src_page_stride_bytes=src_stride,
            copy_bytes=src_stride,
        )
        torch.cuda.synchronize()
        for destination_page, source_page in zip(
            destination_pages.cpu(), source_pages.cpu()
        ):
            dst_start = destination_page * dst_stride
            src_start = source_page * src_stride
            torch.testing.assert_close(
                arena.raw[dst_start : dst_start + src_stride],
                source[src_start : src_start + src_stride].cpu(),
            )
            assert torch.all(
                arena.raw[dst_start + src_stride : dst_start + dst_stride] == 255
            )
    finally:
        arena.destroy()
...
def test_shared_typed_chunk_kv_mamba_roundtrip(dtype):
    ...
    try:
        ...
        # Device Mamba uses the production unified-L1 raw slot envelope:
        # [conv rows x layers][temporal rows x layers].
        device_mamba_raw = torch.zeros(
            device_slots * mamba_entry_bytes, dtype=torch.uint8, device="cuda"
        )
        conv_views, temporal = build_page_major_mamba_views(
            ...
            max_slots=device_slots,
        )
        conv = conv_views[0]
        mamba_unified_marker = SimpleNamespace(
            total_bytes=device_mamba_raw.numel(), _raw=device_mamba_raw
        )
        mamba_device = SimpleNamespace(
            _unified_buffer=mamba_unified_marker,
