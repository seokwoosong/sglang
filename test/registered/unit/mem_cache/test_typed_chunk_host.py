...
class TestTypedChunkHostAllocator(CustomTestCase):
    ...
    def test_kv_suballocation_and_empty_chunk_retyping(self):
        ...
        mamba = alloc.alloc_mamba(1)
        self.assertEqual(mamba.tolist(), [0])
    ...
    def test_mamba_uses_exactly_one_chunk_per_slot(self):
        ...
        kv = alloc.alloc_kv(4)
        self.assertEqual(kv.tolist(), [4, 5, 6, 7])
        self.assertEqual(alloc.owner(1), HostChunkOwner.KV)
        alloc.assert_consistent()
    ...
    def test_pool_adapters_share_one_arena_and_retype_empty_chunks(self):
        ...
        with (
            mock.patch(
                "sglang.srt.mem_cache.pool_host.unified_chunk.can_use_hicache_jit_kernel",
                return_value=True,
            ),
            mock.patch(
                "sglang.srt.mem_cache.pool_host.unified_chunk.can_use_write_back_jit_kernel",
                return_value=True,
            ),
        ...
        self.assertEqual(mamba_indices.tolist(), [1])
        kv_host.kv_buffer[:, kv_indices].fill_(7)
        mamba_host.temporal_buffer[mamba_indices].fill_(11)
        mamba_host.conv_buffer[0][mamba_indices].fill_(13)

        self.assertEqual(arena.chunks.owner(0), HostChunkOwner.KV)
        self.assertEqual(arena.chunks.owner(1), HostChunkOwner.MAMBA)
        torch.testing.assert_close(
            mamba_host.get_data_page(1),
            torch.cat(
                [
                    mamba_host.temporal_buffer[1]
                    .contiguous()
                    .view(torch.uint8)
                    .flatten(),
                    mamba_host.conv_buffer[0][1]
                    .contiguous()
                    .view(torch.uint8)
                    .flatten(),
                ]
        ...
        # L3 storage serializes through get_data_page/set_from_flat_data_page.
        # Exercise both shared views with non-uniform bit patterns so an
        # incorrect stride, component offset, or dtype conversion cannot hide
        # behind an all-constant tensor comparison.
        kv_word_count = kv_host.get_dummy_flat_data_page().numel()
        kv_payload = torch.arange(kv_word_count, dtype=torch.int16).view(torch.float16)
        kv_host.set_from_flat_data_page(0, kv_payload)
        kv_serialized = kv_host.get_data_page(0).clone()
        self.assertTrue(
            torch.equal(kv_serialized.view(torch.uint8), kv_payload.view(torch.uint8))
        )
        kv_host.kv_buffer[:, 0].zero_()
        kv_host.set_from_flat_data_page(0, kv_serialized)
        self.assertTrue(
            torch.equal(
                kv_host.get_data_page(0).view(torch.uint8),
                kv_payload.view(torch.uint8),
            )
        )

        mamba_byte_count = mamba_host.get_dummy_flat_data_page().numel()
        mamba_payload = torch.arange(mamba_byte_count, dtype=torch.uint8)
        mamba_host.set_from_flat_data_page(1, mamba_payload)
        mamba_serialized = mamba_host.get_data_page(1).clone()
        self.assertTrue(torch.equal(mamba_serialized, mamba_payload))
        mamba_host.temporal_buffer[1].zero_()
        mamba_host.conv_buffer[0][1].zero_()
        mamba_host.set_from_flat_data_page(1, mamba_serialized)
        self.assertTrue(torch.equal(mamba_host.get_data_page(1), mamba_payload))

        # The file backend's zero-copy path consumes raw pointers rather than
        # the flat fallback above. Verify that those pointers address the same
        # typed chunk and exclude Mamba alignment padding.
        kv_ptrs, kv_sizes = kv_host.get_page_buffer_meta(
            torch.tensor([0], dtype=torch.int64)
        )
        self.assertEqual(kv_ptrs, [arena.raw.data_ptr()])
        self.assertEqual(kv_sizes, [arena.chunks.kv_page_bytes])

        mamba_ptrs, mamba_sizes = mamba_host.get_page_buffer_meta(
            torch.tensor([1], dtype=torch.int64)
        )
        expected_mamba_ptrs = [
            mamba_host.temporal_buffer.data_ptr()
            + mamba_host.temporal_buffer.stride(0)
            * mamba_host.temporal_buffer.element_size(),
            mamba_host.conv_buffer[0].data_ptr()
            + mamba_host.conv_buffer[0].stride(0)
            * mamba_host.conv_buffer[0].element_size(),
        ]
        expected_mamba_sizes = [
            mamba_host.temporal_buffer[0].numel()
            * mamba_host.temporal_buffer.element_size(),
            mamba_host.conv_buffer[0][0].numel()
            * mamba_host.conv_buffer[0].element_size(),
        ]
        self.assertEqual(mamba_ptrs, expected_mamba_ptrs)
        self.assertEqual(mamba_sizes, expected_mamba_sizes)
        self.assertEqual(sum(mamba_sizes), arena.chunks.mamba_slot_bytes)

        # Freeing both KV pages empties chunk 0, which Mamba can immediately own.
        kv_host.free(kv_indices)
        next_mamba = mamba_host.alloc(1)
        self.assertEqual(next_mamba.tolist(), [0])
